...
import { BasicSidebarExample, SidebarWithNestedNavigationExample, SidebarWithSearchExample, AdminLayoutExample } from '@/components/application-shells/SidebarLayoutExamples';

const SidebarLayouts = () => {
  const [activeFullscreenExample, setActiveFullscreenExample] = useState<string | null>(null);
  
  const openFullscreen = (exampleId: string) => {
    setActiveFullscreenExample(exampleId);
  };
  ...
  return (
    <div className="w-full animate-fade-in">
      <Header 
        ...
        description="Layouts de sidebar são estruturas fundamentais de interfaces que proporcionam navegação lateral, usualmente combinada com um header e conteúdo principal." 
        type="components" 
      />
      
      <div className="mt-8 space-y-12">
        <ComponentCard 
          title="Layout de Sidebar Básico" 
          description="Layout de sidebar simples com navegação lateral e conteúdo principal."
          code={basicSidebarCode}
          showCodeBlockInside={true}
          codeBlockTitle="Layout de Sidebar Básico"
          codeBlockLanguage="tsx"
        >
          <div className="w-full p-4 flex justify-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
            <div className="border border-gray-200 shadow-sm rounded-lg overflow-hidden w-full max-w-4xl" style={{ height: '400px' }}>
              <BasicSidebarExample />
            </div>
            <Button 
              variant="outline" 
        ...
        <ComponentCard 
          title="Sidebar com Navegação Aninhada" 
          description="Layout de sidebar com menus expansíveis e navegação multinível."
          code={nestedNavigationCode}
          showCodeBlockInside={true}
          codeBlockTitle="Sidebar com Navegação Aninhada"
          codeBlockLanguage="tsx"
        >
          <div className="w-full p-4 flex justify-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
            <div className="border border-gray-200 shadow-sm rounded-lg overflow-hidden w-full max-w-4xl" style={{ height: '400px' }}>
              <SidebarWithNestedNavigationExample />
            </div>
            <Button 
              variant="outline" 
              size="sm" 
              className="absolute top-4 right-4"
              onClick={() => openFullscreen('nested')}
            >
              <Maximize2 className="h-4 w-4 mr-1" />
              <span>Tela cheia</span>
            </Button>
        ...
        <ComponentCard 
          title="Sidebar com Busca" 
          description="Layout de sidebar com funcionalidade de busca integrada."
          code={searchSidebarCode}
          showCodeBlockInside={true}
          codeBlockTitle="Sidebar com Busca"
          codeBlockLanguage="tsx"
        >
          <div className="w-full p-4 flex justify-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
            <div className="border border-gray-200 shadow-sm rounded-lg overflow-hidden w-full max-w-4xl" style={{ height: '400px' }}>
              <SidebarWithSearchExample />
            </div>
            <Button 
              variant="outline" 
              size="sm" 
        ...
        <ComponentCard 
          ...
          description="Layout completo com sidebar, cabeçalho e área de conteúdo principal, adequado para painéis administrativos."
          code={adminLayoutCode}
          showCodeBlockInside={true}
          codeBlockTitle="Layout Administrativo Completo"
          codeBlockLanguage="tsx"
        >
          <div className="w-full p-4 flex justify-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
            <div className="border border-gray-200 shadow-sm rounded-lg overflow-hidden w-full max-w-4xl" style={{ height: '400px' }}>
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-semibold">Melhores Práticas para Layouts de Sidebar</h2>
          <ul className="list-disc pl-6 space-y-2">
            <li className="text-gray-700">Mantenha a navegação principal na sidebar para fácil acesso</li>
            <li className="text-gray-700">Agrupe itens relacionados em seções ou categorias</li>
            <li className="text-gray-700">Ofereça um mecanismo para recolher/expandir a sidebar em telas pequenas</li>
            <li className="text-gray-700">Use ícones consistentes para melhorar a usabilidade</li>
            <li className="text-gray-700">Destaque claramente o item ativo na navegação</li>
            <li className="text-gray-700">Considere usar rótulos para seções principais</li>
            <li className="text-gray-700">Mantenha a profundidade da navegação limitada (2-3 níveis no máximo)</li>
          </ul>
        </div>
      </div>
      
      {/* Fullscreen Dialog */}
      <Dialog open={activeFullscreenExample !== null} onOpenChange={closeFullscreen}>
        <DialogContent className="max-w-7xl w-[95vw] max-h-[90vh] h-[90vh] p-0">
          <div className="w-full h-full overflow-hidden">
            {activeFullscreenExample === 'basic' && <BasicSidebarExample />}
            {activeFullscreenExample === 'nested' && <SidebarWithNestedNavigationExample />}
            {activeFullscreenExample === 'search' && <SidebarWithSearchExample />}
            {activeFullscreenExample === 'admin' && <AdminLayoutExample />}
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
const nestedNavigationCode = `import React from 'react';
import { Home, Users, Settings, ChevronDown, ChevronRight, FileText, ShoppingCart, Mail, Menu, X } from 'lucide-react';

export const SidebarWithNestedNavigation = () => {
  const [isOpen, setIsOpen] = React.useState(true);
...
import { Home, Users, Settings, Search, Bell, FileText, ShoppingCart, Mail, Menu, X } from 'lucide-react';
...
import { Home, Users, Settings, Search, Bell, Menu, X, ChevronDown, User, LogOut } from 'lucide-react';
